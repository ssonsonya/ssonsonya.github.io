...
3.  레벨 애셋 적용 후 랜더링 상태   
    ...
    | :---: | :---: | :---: | :---: |
    | 12.5GB (+8GB) | 1,698.98 | 27,147,798 | 80.15ms (약 12.5 fps) |
...
+ 뷰포트에서의 DrawCall / Triangles Drawn 확인  
    ...
    \`cmd 호출 > `stat RHI`  
    RHI(Render Hardware Interface : 지원하는 플랫폼의 각기다른 그래픽 API에 대한 크로스플랫폼 인터페이스) 메모리와 통계와 성능 확인  
...
+ 프리미티브 통계정보 확인  
    ...
    가장 크기가 큰 액터 또는 tri수가 많은 액터를 확인할 수 있다.
...
우선 가장 시급해보이는 폴리곤수를 줄이기 위해 Mesh의 LOD를 설정했다.  
...
언리얼 엔진에서의 랜더링 구조를 알아가며 큰 범위에서 부터 점점 범위를 좁혀가며 적절한 최적화 방법을 적용하는 것이 효율적이라는 것을 알 수 있었다.  
...
[^22]: [Unreal Engine. Unreal Engine Documentation. 자동 LOD 생성 셋업](https://dev.epicgames.com/documentation/ko-kr/unreal-engine/setting-up-automatic-lod-generation?application_version=4.27)  
