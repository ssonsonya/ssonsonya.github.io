...
각 Frame 당 Curve Key가 입력되어야 하므로 Animation Sequence의 Frame수만큼 For Loop를 돌며 시작한다
...
- Test Mode  
    ...
    *축이 바뀌어있는 Skeleton의 경우 오류를 확인 할 수 있다*  
    ...
    2. 시간을 입력해 줘야 하지만 Frame을 기준으로 위치값을 구했기 때문에[`GetTimeatFrame`](https://docs.unrealengine.com/4.26/en-US/BlueprintAPI/AnimationBlueprintLibrary/Helpers/GetTimeatFrame/) [^252] 를 통해 Loop로 돌고 있는 CurrentFrameIndex값을 시간으로 계산해 AddFloatCurveKey에 입력해준다
    3. BoneHeightsByFrame 배열의 해당 순서의 값을 Key value로 입력해준다

![image](https://github.com/ssonsonya/ssonsonya.github.io/assets/116151781/818ef21b-d436-46b9-ae55-99dd5c4136ae)

- Add Key & Notify Event
    Test Mode를 통해 오류여부를 확인한 뒤 특이사항이 없으면 실제 필요한 Key값과 Notify Event를 추가해준다
