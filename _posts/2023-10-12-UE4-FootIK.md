...
2. Blueprint의 경우) FootIKData를 담아줄 구조체 생성
...
+ Variables
    ...
    - `(struct)FootIKData FootIKData` : IKComponent에서 계산 완료되어 ABP로 전달해 줄 Transform 데이터
        ...
        - `vector IKEffector_L` : Foot_L에 Effector Location으로 적용될 벡터
        - `vector IKEffector_R` : Foot_R에 Effector Location으로 적용될 벡터
        - `rotator Rot_L` : Foot_L에 적용할 Rotator값
        - `rotator Rot_R` : Foot_R에 적용할 Rotator값
    - `float Displacement` : `DisplacementZ`에 set 될 Pelvis가 Z이동해야 할 값
    - `float LFootEffector` : `IKEffector_L.X`에 set 될 Foot_L과 지면의 높이 차
    - `float RFootEffector` : `IKEffector_R.X`에 set 될 Foot_R과 지면의 높이 차(위아래가 뒤집어진 Foot_R본으로 인해 * -1.0f)
    - `rotator LFootRotator` : `Rot_L`에 set 될 지면 Normal 벡터의 역각
    - `rotator RFootRotator` : `Rot_R`에 set 될 지면 Normal 벡터의 역각
    - `float CharacterScale` : Scale적용으로 인한 오차계산용 `OwnerCharacter`의 `ActorWorldScale3D.Z`

+ Functions
    - `LineTrace` : 입력받는 socket이름에서부터 지면을 향해 수직 Line Trace 한 뒤 Hit된 지점의 `Impact Point`와 `Normal`로 발의 위치와 지면의 거리, 역각을 반환해줄 함수
...
10. `Scale`이 크거나 작은 Skeletal Mesh의 경우를 위해 `CharacterScale`값을 각 발에 나눠줌
    - Scale이 커진 경우 해당 Scale만큼 발과 지면의 WorldSpace상에서의 거리차가 커지지만 실제 BoneSpace에서는 변화가 없으므로 Scale을 역산해준다  


![Scale applied](https://github.com/ssonsonya/ssonsonya.github.io/assets/116151781/2191c7cc-3e57-488d-9cfe-77e82aff45fb)
