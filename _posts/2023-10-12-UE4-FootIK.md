...
+ Variables
    ...
    - `float LFootEffector` : `FootIKData.IKEffector_L.X`에 set 될 Foot_L과 지면의 높이 차
    - `float RFootEffector` : `FootIKData.IKEffector_R.X`에 set 될 Foot_R과 지면의 높이 차(위아래가 뒤집어진 Foot_R본으로 인해 * -1.0f)
    - `rotator LFootRotator` : `FootIKData.Rot_L`에 set 될 지면 Normal 벡터의 역각
    - `rotator RFootRotator` : `FootIKData.Rot_R`에 set 될 지면 Normal 벡터의 역각
