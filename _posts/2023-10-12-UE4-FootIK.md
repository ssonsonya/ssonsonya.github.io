...
void UCFootIKComponent::LineTrace(FName InSocketName, float& OutFootTraceZ, FRotator& OutFootRot)
{
    ...
    float z1 = OwnerCharacter->GetActorLocation().Z;
    FVector start = FVector(socketLocation.X, socketLocation.Y, z1);

    // Trace End
    FVector meshLocation = OwnerCharacter->GetMesh()->GetWorldLocation();
    float z2 = meshLocation.Z - OwnerCharacter->GetCharacterMovementComponent().MaxStepHeight;
    FVector end = FVector(socketLocation.X, socketLocation.Y, z2);
